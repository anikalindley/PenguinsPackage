...
The package `project3package` contains four functions. `my_t.test` is used to make statistical inferences,  `my_lm` is used to make predictions, and `my_knn_cv` and `my_rf_cv` use cross validation to assess the predictive performance of models made using k-nearest neighbors and random forest, respectively.
...
Based on the training missclassification rate, I would choose the model where we use 1 nearest neighbor, because the training error rate is `r training_err[1]`. Based on the CV misclassification rates, I would also choose the model with 1 nearest neighbor, because the error rate is the lowest. The CV misclassification error uses 5-fold cross validation to determine how well the estimations predict the true class. While the model decision is the same using both models, I would use the CV misclassification error to justify this deicison, becasue the training missclassification error for one nearest neighbor is always 0 since the first nearest neighbor is the data point itself. 
